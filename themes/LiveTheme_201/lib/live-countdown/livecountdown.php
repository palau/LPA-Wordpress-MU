...
Changelog:
- 1.5: Timezone compliant
- 1.1: Updating the hours, minutes, and second values to avoid an NaN error on the client-side

*/

/*  Copyright 2010 - 2011 8BIT  (email : info@8bit.io)
...
class Live_Countdown_Widget extends WP_Widget {
	...
	function Live_Countdown_Widget() {
      ...
      $bumper_duration = $standard_options->bumper_duration ? $standard_options->bumper_duration : "5";
      
			wp_enqueue_style('livecountdown',  get_template_directory_uri() . '/lib/live-countdown/css/livecountdown.css');
			wp_enqueue_script('jquery.countdown',  get_template_directory_uri() . '/lib/live-countdown/javascript/jquery.countdown.min.js');
			wp_enqueue_script('jquery.livecountdown',  get_template_directory_uri() . '/lib/live-countdown/javascript/jquery.livecountdown.js?bumper_duration=' . $bumper_duration);
	...
	function widget($args, $instance) {
		...
		<div class="live_countdown_container">
			...
			<div class="live_countdown_control" style="display:none;">
				...
				<span class="live_countdown_minute"><?php echo strlen($minute) == 6 ? 0 : $minute; ?></span>
				<span class="live_countdown_ampm"><?php echo $ampm; ?></span>
				<span class="live_countdown_timezone"><?php echo $timezone; ?></span>
				<span class="live_countdown_url"><?php echo get_template_directory_uri() . '/lib/live-countdown/php/livecountdown.time.php'; ?></span>
			</div>
		</div>
		<?php
	...
	?>
		<p>
			...
			<select id="<?php echo $this->get_field_id('month'); ?>" name="<?php echo $this->get_field_name('month'); ?>" class="live_countdown_month">
				<?php for($i = 1; $i <= 12; $i++): ?>
			...
			<select id="<?php echo $this->get_field_id('day'); ?>" name="<?php echo $this->get_field_name('day'); ?>" class="live_countdown_day">
				<?php for($i = 1; $i <= 31; $i++): ?>
			...
			<select id="<?php echo $this->get_field_id('year'); ?>" name="<?php echo $this->get_field_name('year'); ?>" class="live_countdown_year">
		...
		<p>
			...
			<input type="text" id="<?php echo $this->get_field_id('timezone'); ?>" name="<?php echo $this->get_field_name('timezone'); ?>" value="<?php echo $instance['timezone']; ?>" class="live_countdown_timezone" style="display:none;" />
			<script type="text/javascript">
			var x = setInterval(function() {
				if(jQuery('.live_countdown_timezone:last').val() === '') {
					...
					jQuery('.live_countdown_timezone:last').val(sOffsetQualifier + sOffset);
					clearInterval(x);
				} // end if
			}, 3000);
			</script>
