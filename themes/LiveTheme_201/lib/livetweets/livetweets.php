...
class Live_Tweets_Widget extends WP_Widget {
    ...
    $displaytag = empty($instance['displaytag']) ? '&nbsp;' : apply_filters('widget_displaytag', $instance['displaytag']);
    ...
    <?php if($displaytag == 'on' && strlen(trim($hashtag)) > 0) { ?>
      <div class="live_tweets_hashtag_notice">
        <?php
          _e("Use Hashtag &#35;", "live_tweets");
          echo $hashtag; 
         ?>
      </div>
    <?php } // end if ?>
		...
		<input type="hidden" id="live_tweets_username" class="live_tweets_username" value="<?php echo substr($username, 0, 1) == '@' ? '' : trim($username); ?>" id="<?php echo $this->get_field_id('username'); ?>" name="<?php echo $this->get_field_name('username');; ?>" />
    ...
    $instance['displaytag'] = strip_tags(stripslashes($new_instance['displaytag']));
    ...
    $displaytag = strip_tags(stripslashes($instance['displaytag']));
	...
	?>
		...
		<div>
			...
			<input type="text" value="<?php echo attribute_escape($updaterate); ?>" id="<?php echo $this->get_field_id('updaterate'); ?>" name="<?php echo $this->get_field_name('updaterate'); ?>" class="widefat" style="margin-bottom: 4px;" />
      <label for="displaytag">
				<?php _e("Display Hashtag?", "live_tweets"); ?>
			</label>
      <input type="checkbox" id="<?php echo $this->get_field_id('displaytag'); ?>" name="<?php echo $this->get_field_name('displaytag'); ?>" <?php ?> <?php echo $displaytag == 'on' ? "checked='checked'" : ''; ?> />
		</div>
	<?php
	} // end form
