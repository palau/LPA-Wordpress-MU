...
function wptouch_get_mobile_switch_link() {
	...
	if ( $settings->show_switch_link ) {
		if ( $settings->home_page_redirect_address == 'same' ) {
			return apply_filters( 'wptouch_mobile_switch_link', get_bloginfo( 'home' ) . '?wptouch_switch=desktop&amp;redirect=' . urlencode( $_SERVER['REQUEST_URI'] ) );
		} else {
			return apply_filters( 'wptouch_mobile_switch_link', get_bloginfo( 'home' ) . '?wptouch_switch=desktop&amp;redirect=' . get_bloginfo( 'home' ) );
		}
	}
}
...
function wptouch_get_desktop_switch_link() {
	...
	if ( $settings->show_switch_link ) {
		if ( $settings->home_page_redirect_address == 'same' ) {
			return apply_filters( 'wptouch_desktop_switch_link', get_bloginfo( 'home' ) . '?wptouch_switch=mobile&amp;redirect=' . urlencode( $_SERVER['REQUEST_URI'] ) );
		} else {
			return apply_filters( 'wptouch_desktop_switch_link', get_bloginfo( 'home' ) . '?wptouch_switch=mobile&amp;redirect=' . get_bloginfo( 'home' ) );
		}
	}
}
...
function wptouch_has_welcome_message() {
	$settings = wptouch_get_settings();
	if ( isset( $_COOKIE['wptouch_welcome'] ) ) {
...
function wptouch_get_welcome_message_dismiss_url() {
	return apply_filters( 'wptouch_welcome_message_dismiss_url', get_bloginfo( 'home' ) . '?wptouch_pro=dismiss_welcome&amp;redirect=' . $_SERVER['REQUEST_URI'] );
}
